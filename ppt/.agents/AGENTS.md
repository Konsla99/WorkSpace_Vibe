...
*   **단일 진실 공급원(Single Source of Truth) 준수**:
    *   포트폴리오의 정보(이름, 소개글, 경력, 학력, 기술 스택, 프로젝트 정보 등)는 오직 [portfolio_input.md](file:///D:/workspace_Gemini/ppt/portfolio_input.md)를 기준으로 관리합니다.
    *   사용자가 [portfolio_input.md](file:///D:/workspace_Gemini/ppt/portfolio_input.md)의 내용을 수정 또는 추가하고 반영을 요청하는 경우, 에이전트가 이를 직접 읽어 [portfolio.html](file:///D:/workspace_Gemini/ppt/portfolio.html)과 [portfolio_PDF.html](file:///D:/workspace_Gemini/ppt/portfolio_PDF.html)에 구조를 깨뜨리지 않고 정교하게 수동 반영해야 합니다.
*   **단일 파일 유지**: 모든 스타일과 스크립트는 `portfolio.html` 및 `portfolio_PDF.html` 내부의 `<style>` 및 `<script>` 태그 내에 작성하며, 외부 CSS/JS 파일로 분리하지 않습니다.
...
*   **인쇄(PDF) 대응 및 두 파일의 차이점**:
    ...
    *   [portfolio.html](file:///D:/workspace_Gemini/ppt/portfolio.html)은 `.main-wrapper`의 배경색을 `var(--wrapper-bg)` (회색)으로 설정합니다.
    *   [portfolio_PDF.html](file:///D:/workspace_Gemini/ppt/portfolio_PDF.html)은 `.main-wrapper`의 배경색을 `#FFFFFF` (흰색)으로 설정하여 PDF 출력 시 회색 배경이 나타나지 않도록 유지합니다.
